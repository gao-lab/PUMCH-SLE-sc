...
load("./output_file/seurat/plasma/plasma_filter_remove_ss_recluster_without_harmony.rdata")
all_bcr <- readRDS("./output_file/scRepertoire/BCR/all_BCR_combined.rds")

# We need to handle the barcode difference between scRepertoire and seurat
# SAMPLE + GROUP + BARCODE
Cells(plasma_filter) %>% head()
scRepertoire_barcode <- plasma_filter@meta.data %>% rownames_to_column("barcode") %>%
