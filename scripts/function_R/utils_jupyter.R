...
# @top_genes     : parameter in rawParse(), set 1-100
# >>> iTalk_data and highly exp genes for iTALK analysis
seu_to_iTALK <- function(seu_obj, cell_type = "subtype", compare_group = "disease",
                         top_genes = 50) {
  downsample_count_mat <- sparse_to_dense(dgc_matrix = seu_obj@assays$RNA@counts, format = "DF")
...
# @format        : which format should function return(data.table, data.frame or matrix)
...
sparse_to_dense <- function(dgc_matrix, format = "data.table", file_name = NULL,
  ...
  if (format %in% c("matrix", "Matrix", "mat", "M")) {
    print("return matrix")
    expr <- as.matrix(expr)
  ...
  }else if (format %in% c("data.frame", "Data.Frame", "df", "DataFrame", "DF")) {
...
# @seu_obj    : a seurat object which scaled and clustered
...
# @plot_length: how many top gsea results should be plot(20)
...
# @seu_obj  : a seurat object
# @scale_all: use all of the genes to scale(recommend False to save memory
#             and speed up)m, but necessary when using DoHeatmap()
# @res      : resolution of the cluster, can set a vector like c(0.5, 0.8, 1)
# @qc       : if calculate MT and ribosome gene proportion
